...
class StockInstaller extends Installer {
  ...
  Future<bool> canInstallSilently(App app) async {
    if (isObtainiumVariant(app.id)) {
      AppLogger.info(
        'App will not be installed silently: Discoverium cannot silently install itself: ${app.id}',
      );
      return false;
    }
    ...
    if (installerPackageName == null ||
      ...
      AppLogger.info(
        'App will not be installed silently: Discoverium is not the installing package (current installer: ${installerPackageName ?? 'unknown'}): ${app.id}',
      );
      return false;
    }
